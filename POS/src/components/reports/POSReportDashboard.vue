  ...
  <div class="flex flex-col h-full bg-gray-50" dir="rtl">
    ...
    <div class="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between gap-3 flex-shrink-0">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-base font-bold text-gray-900">{{ __("التقارير") }}</h2>
    ...
    <div class="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
      ...
      <div v-if="!selectedProfile && !filtersLoading" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="text-sm font-semibold text-gray-500">{{ __("لا توجد ملفات POS متاحة") }}</p>
        <p class="text-xs text-gray-400 mt-1">{{ __("لا تملك صلاحية الوصول لأي ملف نقاط بيع") }}</p>
      </div>
      ...
      <template v-else-if="selectedProfile">
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <div class="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-1 col-span-2 min-h-24">
            ...
            <template v-else>
              <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __("إجمالي المبيعات") }}</span>
              ...
              <p class="text-xs text-gray-500">{{ __("{0} فاتورة", [summary.sales_count]) }}</p>
            </template>
          </div>
          ...
          <div class="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-1 min-h-24">
            ...
            <template v-else>
              <span class="text-xs font-semibold text-gray-500">{{ __("متوسط الفاتورة") }}</span>
          ...
          <div class="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-1 min-h-24">
            ...
            <template v-else>
              <span class="text-xs font-semibold text-gray-500">{{ __("صافي المبيعات") }}</span>
          ...
          <div v-if="!summaryLoading && summary.returns_count > 0"
            ...
            <span class="text-xs font-semibold text-red-600">{{ __("المرتجعات") }}</span>
            <p class="text-xl font-bold text-red-700">{{ fmt(summary.returns_total) }}</p>
            <p class="text-[10px] text-red-400">{{ __("{0} مرتجع", [summary.returns_count]) }}</p>
          </div>
          ...
          <div v-if="!summaryLoading && summary.outstanding_total > 0"
            ...
            <span class="text-xs font-semibold text-amber-700">{{ __("مبيعات آجلة / غير مسددة") }}</span>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-4">
          <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">{{ __("طرق السداد") }}</h3>
          ...
          <template v-else-if="!payment.methods?.length">
            <p class="text-xs text-gray-400 text-center py-4">{{ __("لا توجد بيانات سداد في هذه الفترة") }}</p>
          </template>
          <template v-else>
            <div class="flex flex-col gap-2">
              <div v-for="m in payment.methods" :key="m.mode"
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] text-gray-400 flex-shrink-0">{{ __("{0} عملية", [m.count]) }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-4">
          <div class="flex items-center justify-between mb-3">
            <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wide">{{ __("آخر العمليات") }}</h3>
            <span v-if="transactions.count > 0" class="text-[10px] text-gray-400">{{ __("آخر {0}", [transactions.count]) }}</span>
          </div>
          ...
          <template v-else-if="!transactions.transactions?.length">
            <div class="text-center py-8">
              <p class="text-xs text-gray-400">{{ __("لا توجد فواتير في هذه الفترة") }}</p>
            </div>
          </template>
          <template v-else>
              ...
              <div v-for="tx in transactions.transactions" :key="tx.name"
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span v-if="tx.is_return" class="text-[9px] bg-red-100 text-red-600 font-bold px-1 rounded">{{ __("مرتجع") }}</span>
                  </div>
                ...
                <div class="text-end flex-shrink-0">
                  ...
                  <p v-if="tx.outstanding_amount > 0" class="text-[10px] text-amber-600">{{ __("آجل") }}</p>
                </div>
                <!-- Open arrow -->
...
const profileLabel = computed(() => {
  if (!selectedProfile.value) return __("اختر ملف POS");
...
const PAYMENT_ICONS = {
  Cash: "💵", كاش: "💵", نقد: "💵",
  Card: "💳", "Debit Card": "💳", "Credit Card": "💳", مدى: "💳", Mada: "💳", مدي: "💳",
  Transfer: "🏦", "Bank Transfer": "🏦", تحويل: "🏦",
  Wallet: "👛", محفظة: "👛",
};
function paymentIcon(mode) { return PAYMENT_ICONS[mode] || "💰"; }

function fmt(val) {
  if (val === null || val === undefined) return "0.00";
  return parseFloat(val).toLocaleString("ar-SA", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
...
function onFilterChange() {
  ...
  if (selectedPeriod.value === "custom" && (!customFrom.value || !customTo.value)) return;
  loadAll();
}
...
async function loadFilters() {
  ...
  try {
    const res = await frappe.call({
      ...
      error: r => { errorMsg.value = r?.message || __("خطأ في تحميل الفلاتر"); },
    });
    if (res?.message) {
...
async function loadAll() {
  ...
  const [sumRes, payRes, txRes] = await Promise.all([
    frappe.call({ method: "pos_next.api.reports.get_daily_summary", args, error: r => { errorMsg.value = r?.message || __("خطأ في تحميل الملخص"); } }),
    frappe.call({ method: "pos_next.api.reports.get_payment_breakdown", args, error: r => { console.error(r); } }),
    frappe.call({ method: "pos_next.api.reports.get_recent_transactions", args: { ...args, limit: 20 }, error: r => { console.error(r); } }),
  ]);
...
onMounted(async () => {
  ...
  try {
    const shiftRes = await frappe.call({ method: "pos_next.api.reports.get_current_shift_profile" });
...
watch(selectedProfile, () => { if (selectedProfile.value) loadAll(); });
</script>
