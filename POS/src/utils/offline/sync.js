...
export const syncOfflinePayments = async (options = {}) => {
	...
	const pendingPayments = (await getOfflinePayments()).filter((payment) => includeFailed || !payment.sync_failed)
	const result = { success: 0, failed: 0, skipped: 0, errors: [] }

	for (const payment of pendingPayments) {
		try {
			const payload = payment.data || {}
			const response = await call("pos_next.api.partial_payments.add_payment_to_partial_invoice", {
			...
			result.failed++
			result.errors.push({ id: payment.id, offlineId: payment.offline_id, invoiceName: payment.invoice_name, error })
		}
	}
