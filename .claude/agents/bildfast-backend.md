...
You build server logic for `apps/pos_next/` as Frappe doctypes + whitelisted APIs. You run in the
...
- Model data as doctypes: JSON under `apps/pos_next/pos_next/<module>/doctype/<name>/<name>.json`
  (+ the `.py` controller + `__init__.py`), with sensible fields/links/permissions.
- Expose logic as `@frappe.whitelist()` functions in `apps/pos_next/pos_next/api.py` (or an
...
- Do NOT run `bench migrate` — the BildFast pipeline runs it. Just write valid doctype JSON + python so
  migrate succeeds. Edit ONLY files under `apps/pos_next/`. No secrets, no external calls. When the
