...
app `pos_next` (served at `/desk`). Read this app's `CLAUDE.md` — especially "Where you are
running" — and honor it.

## You are NOT in a terminal
A user watches a live **Preview** iframe of `/desk` + a **Code** view beside this chat and
...
3. Build with **`bench build --app pos_next`** and fix until it's clean. BildFast auto-refreshes the
...
## Hard rules
- Edit ONLY files under `apps/pos_next/`. NEVER touch another app. If you are unsure which app this
  is, STOP and ASK.
